...
test('Client tx already in cache test', async t => {
  t.plan(2)
  let client
  try {
    ...
    await waitForMillisecondsAsync(10)
    const r1 = callTransactionAsync(client, caller, address, functionSet)
    await waitForMillisecondsAsync(10)
    const r2 = callTransactionAsync(client, caller, address, functionSet)

    r2.catch(err => t.assert(err.message.indexOf('already exists')))
    r2.catch(err => t.assert(err.message.indexOf('already exists')))

    await waitForMillisecondsAsync(4000)
  } catch (err) {
    console.error(err)
    t.fail(err.message)
  }
