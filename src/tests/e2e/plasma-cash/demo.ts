...
import { IPlasmaDeposit, marshalDepositEvent } from '../../../plasma-cash/ethereum-client'
import { increaseTime } from '../../ganache-helpers'
import { ADDRESSES, ACCOUNTS, DEFAULT_GAS, CHILD_BLOCK_INTERVAL } from './config'
...
test('Plasma Cash with ERC721 Demo', async t => {
  ...
  const authority = new Entity(web3, ACCOUNTS.authority)
  const alice = new Entity(web3, ACCOUNTS.alice)
  const bob = new Entity(web3, ACCOUNTS.bob)
  const charlie = new Entity(web3, ACCOUNTS.charlie)
