...
export class BscTransferGateway extends TransferGateway {
  static async createAsync(client: Client, callerAddr: Address): Promise<BscTransferGateway> {
    const contractAddr = await client.getContractAddressAsync('bsc-gateway')
    if (!contractAddr) {
      throw Error('Failed to resolve contract address for BscTransferGateway')
    }

    return new BscTransferGateway({ contractAddr, callerAddr, client })
  }
